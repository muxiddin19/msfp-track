...
\caption{ROC curves comparing re-identification discriminability across LITE variants. Multi-layer fusion methods (LITE+, LITE++) consistently outperform single-layer LITE, with LITE+ (concat) achieving the highest AUC.}
\label{fig:roc}
\end{figure}
...
\midrule
Single Layer (LITE) & 0.9085 & 0.1023 & 64 \\
Concat + MLP & 0.9178 & 0.1542 & 128 \\
Attention-Weighted & 0.9305 & 0.1691 & 128 \\
LITE++ (Attention + ATL) & \textbf{0.9356} & \textbf{0.1763} & 128 \\
\bottomrule
\end{tabular}
\end{table}

The LITE++ variant with attention fusion and adaptive threshold learning achieves the highest AUC (0.9356) and positive-negative gap (0.1763), demonstrating the combined benefit of multi-scale features and scene-adaptive thresholds. All multi-layer variants outperform single-layer LITE, confirming the value of multi-scale feature fusion.

\subsubsection{Layer Combination}

\begin{table}[t]
\centering
\caption{Ablation on layer combinations for feature extraction (attention fusion).}
\label{tab:layers}
\begin{tabular}{lcc}
\toprule
Layers & AUC $\uparrow$ & Pos-Neg Gap $\uparrow$ \\
\midrule
Layer 14 only (baseline) & 0.9085 & 0.1023 \\
Layer 9 + 14 & 0.9198 & 0.1385 \\
Layer 4 + 14 & 0.9142 & 0.1276 \\
Layer 4 + 9 + 14 & \textbf{0.9305} & \textbf{0.1691} \\
\bottomrule
\end{tabular}
\end{table}

Using all three layers (layer 4, 9, 14) provides the best performance with 2.4\% AUC improvement and 65\% higher positive-negative gap over single-layer LITE, confirming that multi-scale features capture complementary information beneficial for appearance-based association.
...
\begin{table}[t]
\centering
\caption{Speed comparison of ReID variants (ms per frame, CPU inference).}
\label{tab:speed}
\begin{tabular}{lcc}
\toprule
Method & ReID Time (ms) & Overhead \\
\midrule
DeepSORT (separate model) & 45.2 & +681\% \\
StrongSORT (BoT-S50) & 89.7 & +1452\% \\
LITE (single layer) & 5.78 & baseline \\
LITE+ (3 layers, concat) & 5.73 & -1\% \\
LITE+ (3 layers, attention) & 6.71 & +16\% \\
LITE++ (attention + ATL) & 6.69 & +16\% \\
\bottomrule
\end{tabular}
\end{table}

LITE++ adds minimal overhead (+16\%) compared to single-layer LITE while achieving 3\% higher AUC and 72\% larger positive-negative gap. This is far more efficient than traditional ReID models which add 680-1450\% overhead.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/score_distributions.pdf}
\caption{Similarity score distributions for positive (same identity) and negative (different identity) pairs across LITE variants. Multi-layer fusion achieves better separation between positive and negative pairs.}
...
\caption{t-SNE visualization of LITE++ feature embeddings on MOT17-02. Different colors represent different track identities. Features cluster well by identity, demonstrating discriminative power.}
\label{fig:tsne}
\end{figure}
